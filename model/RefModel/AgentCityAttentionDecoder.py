...
class AgentCityAttentionDecoder(nn.Module):
    def __init__(self, hidden_dim=256, embed_dim=128, num_heads=4, num_layers=2, norm = "batch"):
    ...
    def forward(self,agent_embed, city_embed, masks = None):
        ...
        for model in self.agent_self_att:
            agent_embed = model(agent_embed, city_embed,mask = masks)

        logits = self._logp(agent_embed, city_embed, masks)
