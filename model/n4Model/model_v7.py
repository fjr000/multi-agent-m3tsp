...
def gumbel_topk(logits, k_tensor, temperature=1.0, hard=True):
    """
     Args:
         logits:     [B, 1, N]
         k_tensor:   [B, 1, 1], 表示每个样本要选多少个 agent (int tensor)
         temperature: float, softmax 温度
         hard:       bool, 是否返回 one-hot mask
    ...
    # Step 1: 统一维度处理
    B, _, N = logits.shape
    logits_flat = logits.view(B, N)  # [B, N]

    # Step 2: 处理 K 张量，允许 K=0
    k = k_tensor.squeeze(-1).squeeze(-1).long()  # [B]
    k = torch.clamp(k, min=0, max=N)  # 允许 K=0

    # Step 3: 添加 Gumbel 噪声
    gumbels = -torch.empty_like(logits_flat).exponential_().log()
    noisy_logits = (logits_flat + gumbels) / temperature
    scores = F.softmax(noisy_logits, dim=-1)

    # Step 4: 取每个 batch 的 Top-K indices（如果 max_k == 0 则跳过）
    zero_k_mask = k == 0
    if zero_k_mask.all():
        # 所有 K 都是 0，直接返回全 0 mask 和 0 log_probs
        topk_mask = torch.zeros_like(logits)
        log_probs = torch.zeros(B, device=logits.device)
        return topk_mask, log_probs
    ...
    # 否则正常执行 top-k
    topk_values, topk_indices = torch.topk(scores, k=max_k, dim=-1)  # [B, max_k]

    # Step 5: 构造 batch-wise indices 和 valid mask
    batch_indices = torch.arange(B, device=logits.device).view(-1, 1).expand(B, max_k)
    col_indices = torch.arange(max_k, device=logits.device).view(1, -1).expand(B, -1)
    valid_mask = col_indices < k.view(-1, 1)  # [B, max_k]

    # Flatten valid indices，排除 k == 0 的样本
    selected_batch_idx = batch_indices[valid_mask]  # [Total_valid]
    selected_indices = topk_indices[valid_mask]  # [Total_valid]

    # Step 6: 构建 mask
    flat_mask = torch.zeros(B * N, dtype=torch.float32, device=logits.device)
    flat_mask.scatter_(0, selected_batch_idx * N + selected_indices, 1.0)
    topk_mask = flat_mask.view(B, 1, N)  # 恢复成 [B, 1, N]

    if not hard:
        # Soft mode: 使用 soft scores 加权
        topk_mask = topk_mask * scores.view(B, 1, N)

    # Step 7: 计算 log_probs
    log_probs_all = F.log_softmax(logits_flat / temperature, dim=-1)
    selected_log_probs = log_probs_all[selected_batch_idx, selected_indices]
    log_probs = torch.zeros(B, device=logits.device)
    log_probs.index_add_(0, selected_batch_idx, selected_log_probs)

    # 设置 k == 0 的 log_probs 为 0
    log_probs[zero_k_mask] = 0.0
...
class AgentEmbedding(nn.Module):
    ...
    def forward(self, cities_embed, n_depot_embed, graph_embed, agent_state):
        ...
        global_embed = self.global_embed(agent_state[:,:,8:])
...
class AgentSelector(nn.Module):
    def __init__(self, embed_dim, n_heads, hidden_dim):
        ...
        self.graph_embed = nn.Sequential(
            nn.Linear(2*embed_dim, self.embed_dim),
            nn.GELU(),
            nn.LayerNorm(self.embed_dim),
            nn.Linear(self.embed_dim, self.embed_dim),
        )

        self.agents = SingleHeadAttention(embed_dim)


    def forward(self, graph_embed, agents_embed, agents_mask):
        mean_agents_embed = agents_embed.mean(dim=-2, keepdim = True)
        graph = torch.cat([graph_embed, mean_agents_embed], dim=-1)
        graph = self.graph_embed(graph)
        alpha = self.alpha(graph_embed)

        agents_logits = self.agents(graph, agents_embed, agents_mask)
        return alpha, agents_logits
...
class ActionDecoder(nn.Module):
    def __init__(self, hidden_dim=256, embed_dim=128, num_heads=4, num_layers=2, dropout=0, rnn_type='GRU'):
        ...
        self.agent_selector = AgentSelector(embed_dim, num_heads, hidden_dim)
    ...
    def forward(self, agent, city_embed_mean, city_embed, masks, agent_mask = None):
        ...
        active_agent_num = torch.count_nonzero(~agent_mask,dim=-1)
        ...
        # all_inf_mask = torch.all(torch.isinf(action_logits), dim=-1)  # shape [B, N] or [N]
        # inf_indices = torch.nonzero(all_inf_mask)
        # if inf_indices.numel()>0:
        #     pass
        del masks, expand_masks

        return action_logits, self.agent_embed, logp_K, alpha
...
class ActionsModel(nn.Module):
    ...
    def forward(self, agent, mask, info=None):
        ...
        # del city_mask
        agent_mask = info.get('masks_in_salesmen',None)
        actions_logits, agent_embed, alpha_logits, alpha = self.agent_decoder(agent, self.city_embed_mean, self.city_embed, mask, agent_mask)
        ...
        return actions_logits, agent_embed, alpha_logits, alpha
...
class Model(nn.Module):
    ...
    def forward(self, agent, mask, info=None):
        ...
        actions_logits, agents_embed,alpha_logp, alpha = self.actions_model(agent, mask, info)
        acts = None
        if mode == "greedy":
            # 1. 获取初始选择 --------------------------------------------------------
            # if self.step == 0:
        ...
        return actions_logits, agents_logits, acts, acts_no_conflict, masks, alpha_logp, alpha
