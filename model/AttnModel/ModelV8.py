...
class PositionalEncoder(nn.Module):
    def __init__(self, d_model, max_seq_len=100, base = 10000):
        super().__init__()
        self.d_model = d_model

        pos  = torch.arange(max_seq_len, dtype=torch.float32).unsqueeze(1)   # (N,1)
        _2i  = torch.arange(0, d_model, 2, dtype=torch.float32)          # (d/2)
        div_term = base ** (_2i / d_model)                               # (d/2)
        angle    = pos / div_term                                        # (N,d/2)
        self.encoding = torch.empty(max_seq_len, d_model)                         # (N,d)
        self.encoding[:, 0::2] = torch.sin(angle)   # 偶数列  : sin
        self.encoding[:, 1::2] = torch.cos(angle)   # 奇数列  : cos
...
class CityEncoder(nn.Module):
    def __init__(self, input_dim=2, hidden_dim=256, embed_dim=128, num_heads=4, num_layers=2):
        ...
        self.city_self_att = nn.ModuleList(
            [
                CrossAttentionCacheKVLayer(embed_dim,num_heads=num_heads,hidden_size=hidden_dim, batch_norm=False)
...
class AgentEmbedding(nn.Module):
    def __init__(self, input_dim, hidden_dim, embed_dim):
        ...
        self.embed_dim = embed_dim
        # 1. 当前城市位置嵌入 (只做一次线性投影)
        self.cur_pos_proj = nn.Linear(embed_dim, embed_dim, bias=False)
        # 2. 11 维（7+4）状态特征 -> embed_dim   (LayerNorm 放前面更稳定)
        ...
        # 4. depot + graph 预融合 (一次性 cache，可选多 depot)
        self.depot_graph_fuse = nn.Sequential(
            nn.Linear(embed_dim * 2, embed_dim),
            nn.LayerNorm(embed_dim),
            nn.GELU(approximate="tanh")
        )
        self._cached_depot_graph: torch.Tensor | None = None  # [B, 1, E]
        # 5. 最终上下文融合
    ...
    def cache_embed(self, global_depot_embed, n_depot_embed):
        """
        预计算一次 (depot , graph) 融合后的向量，供后续所有 agent 复用。
        """
        fused = torch.cat([global_depot_embed.expand_as(n_depot_embed),
                           n_depot_embed], dim=-1)
        self._cached_depot_graph = self.depot_graph_fuse(fused)  # [B,1,E]
    ...
    def forward(self, cities_embed, graph_embed, agent_state, batch_mask = None):
        ...
        :return:
        """
        B, M, _ = agent_state.shape
        device = cities_embed.device
        E = self.embed_dim
        # (1) 当前位置嵌入 -----------------------------------
        #    agent_state[:,:,1] 是索引，范围 [0,L)
        idx = agent_state[:, :, 1].long()  # [B,M]
        idx_exp = idx.unsqueeze(-1).expand(-1, -1, E)  # [B,M,E]
        cur_pos = torch.gather(cities_embed, 1, idx_exp)  # [B,M,E]
        cur_pos_embed = self.cur_pos_proj(cur_pos)  # [B,M,E]
        # (2) 11 维状态 -> embed_dim -------------------------
        state_embed = self.state_mlp(agent_state[:, :, 2:])  # [B,M,E]
        # (3) 图级嵌入广播 -----------------------------------
        g_embed = self.graph_proj(graph_embed).unsqueeze(1)  # [B,1,E]
        g_embed = g_embed.expand(-1, M, -1)  # [B,M,E]
        # (4) depot+graph cache  -----------------------------
        depot_graph = (self._cached_depot_graph if batch_mask is None
                       else self._cached_depot_graph[batch_mask])  # [B,1,E]
        depot_graph = depot_graph.expand(-1, M, -1)  # [B,M,E]
        # (5) 融合 ------------------------------------------
        fused = torch.cat([depot_graph,
                           cur_pos_embed,
                           state_embed,
                           g_embed], dim=-1)  # [B,M,4E]
        agent_embed = self.fusion_mlp(fused)  # [B,M,E]
        return agent_embed
