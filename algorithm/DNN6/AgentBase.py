...
class AgentBase:
    def __init__(self, args, config, model_class):
        ...
        self.act_lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.act_optim, "min",
                                                                       patience=10000 / args.eval_interval, factor=0.99,
                                                                       min_lr=2e-5)
        self.conf_lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.conf_optim, "min",
                                                                       patience=10000 / args.eval_interval, factor=0.99,
                                                                       min_lr=2e-5)
    ...
    def state_dict(self):
        checkpoint = {
            "actions_model_state_dict":self.model.actions_model.state_dict(),
            "conflict_model_state_dict":self.model.conflict_model.state_dict(),
    ...
    def load_state_dict(self, checkpoint):
        self.model.actions_model.load_state_dict(checkpoint["actions_model_state_dict"])
        self.model.conflict_model.load_state_dict(checkpoint["conflict_model_state_dict"])
