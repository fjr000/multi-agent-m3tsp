...
from model.AttnModel.ModelV7 import Model, Config
from algorithm.Attn.AgentBase import AgentBase
from utils.TensorTools import _convert_tensor
from torch.amp import autocast, GradScaler    # ✅
torch.set_float32_matmul_precision('high')         # 可选：bfloat16 matmul 精度更高

class Agent(AgentBase):
    def __init__(self, args, config):
        super(Agent, self).__init__(args, config, Model)
        self.model.to(self.device)
        self.name = "Attn_AgentV7"
        # 新增
    ...
    def learn(self, buffer):
        ...
        with autocast("cuda",dtype=torch.bfloat16):  # <-- AMP 开始
            ...
            logits, _, _ = self.model(
                _convert_tensor(buffer['states'], device=self.device),
            ...
            loss = act_loss - (self.args.entropy_coef * act_ent)
            loss = loss / self.args.accumulation_steps  # 梯度累积
        # ================= Forward 结束 =================
        # ================= Backward / Update =================
        ...
        # ------- 返回日志 -------
        def chk(x):  # 取标量方便 log
            return None if x is None else (x.item() if isinstance(x, torch.Tensor) else x)

        return {
            "act_loss": chk(act_loss),
            "act_ent": chk(act_ent),
            "grad": chk(pre_grad),
        }
    ...
    def run_batch_episode(self, env, batch_graph, agent_num, eval_mode=False, exploit_mode="sample", info=None):
        ...
        if eval_mode:
            ...
            # adv = self.compute_indv_advs(env_info['costs'], env_info['penalty'][...,1:1+len(act_logp_list)])
            # adv = self.compute_indv_advs(env_info['costs'], env_info['penalty'][...,1:1+len(act_logp_list)])
    ...
    def compute_indv_advs(self, costs, penalty):
        ...
        def convert_to_bta(arr):
            transposed = arr.transpose(2, 0, 1)

            # Reshape into [B*T, A] by stacking T-slices vertically
            reshaped = transposed.reshape(-1, arr.shape[1])
            return reshaped
        adv_final = convert_to_bta(adv)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--grad_max_norm", type=float, default=0.0)
