...
class Agent(AgentBase):
    ...
    def run_batch_episode(self, env, batch_graph, agent_num, eval_mode=False, exploit_mode="sample", info=None):
        ...
        if eval_mode:
            ...
            # adv2 = self.compute_advs(env_info['costs'], len(act_logp_list), agent_num / graph.shape[1] * min_distance, env_info['conflict_count'])
    ...
    def compute_indv_advs(self, costs, penalty):
        ...
        final_rewards_8 = final_rewards.reshape(final_rewards.shape[0] // 8, 8, agent_num, -1)
        adv = (final_rewards_8 - final_rewards_8.mean(axis = (1,2,3), keepdims = True)) / (final_rewards_8.std(axis = (1,2,3), keepdims = True)+1e-8)
