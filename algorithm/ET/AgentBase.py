...
class AgentBase:
    ...
    def __get_action_logprob(self, states, mode="greedy"):
        actions_logits, agents_logits, acts, acts_no_conflict, agents_mask = self.model(states,mode)
    ...
    def __get_loss(self, act_logp, agents_logp, costs):
        ...
        act_loss = (act_logp_8 * adv_t).mean()
    ...
    def run_batch_episode(self, env, batch_graph, agent_num, eval_mode=False, exploit_mode="sample", info=None):
        ...
        #
        state = env.reset(batch_graph, agent_num)
        self.reset_graph(state.graph, agent_num)
        act_logp_list = []
        agents_logp_list = []
        act_ent_list = []
        agt_ent_list = []
        ...
        while not done:
            state_t = state.n2t(device=self.device)
            if eval_mode:
                acts, acts_no_conflict = self.exploit(state_t, exploit_mode)
            else:
                acts, acts_no_conflict, act_logp, agents_logp, act_entropy, agt_entropy = self.predict(state_t)
            ...
            states, r, done, env_info = env.step(acts_no_conflict)
    ...
    def eval_episode(self, env, batch_graph, agent_num, exploit_mode="sample", info=None):
        with torch.no_grad():
            ...
            trajectory = eval_info["trajectories"]._to_trajs()
