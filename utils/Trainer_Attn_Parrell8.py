...
from model.AttnModel.ModelV7 import Config as Config
...
def worker_process(share_agent, agent_class, args, env_class, env_config, recv_pipe, queue, worker_id):
    ...
    while True:
        ...
        new_state_dict = share_agent.model.state_dict()
        work_agent.model.load_state_dict(new_state_dict)
...
def train_process(share_agent, args, agent_class, agent_args, send_pipes, queue):
    ...
    for epoch in range(start_epoch, args.n_epoch):
        ...
        for i in tqdm.tqdm(range(_start_step, step_epoch), mininterval=1):
            ...
            del buffer
            new_state_dict = train_agent.model.state_dict()
            share_agent.model.load_state_dict(new_state_dict)
            # if total_step % 50 == 0:
            #     torch.cuda.empty_cache()
...
class SharelWorker:
    def __init__(self, agent_class, args, env_class):
        ...
        self.queue = mp.Queue(maxsize=self.num_worker * 2)
