...
def worker_process(share_agent, agent_class, args, env_class, env_config, recv_pipe, queue):
    ...
    while True:
        ...
        features_nb, actions_nb, returns_nb, reward_nb, masks_nb, dones_nb, logp_nb = work_agent.run_batch(env, graph, args.agent_num,
                                                                              args.batch_size // args.num_worker)
        queue.put((graph, features_nb, actions_nb, returns_nb, reward_nb, masks_nb, dones_nb, logp_nb))
...
def train_process(share_agent, agent_class, agent_args, send_pipes, queue, eval_model_pipe, eval_result_pipe):
    ...
    for _ in tqdm.tqdm(range(100_000_000)):
        ...
        for i in range(agent_args.num_worker):
            graph, features_nb, actions_nb, returns_nb, reward_nb, masks_nb, dones_nb, logp_nb = queue.get()
